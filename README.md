...
При компиляции без флагов можно наблюдать увеличение скорости практически в два раза по сравнению с компиляцие без флагов и AVX. Но с флагами -O3 и -Ofast наблюдается ускорение в целых 5 раз по сравнению с эталонным временем работы (-O3 без AVZ). Разница времени работы между компиляцией без флагов и с флагами также возросла. Это может указывать на то, что оптимизатор более приспособлен для программ с SIMD инструкциями.
...
Результаты очень схожи с результатами из прошлого раздела. Точно так же, версия без флагов получается примерно в 1.5 раза медленне чем эталон, а -Ofast очень слабо отличается от -O3 в лучшую сторону.
...
В данном тесте мы наблюдаем достаточно интересную картину. Без флагов оптимизации работа программы практически не ускоряется, что само по себе является странным. Ещё более странным является ускорение программы практически в 5 раз по сравнению с эталоном при компиляции с флагами оптимизации.
